...
//    use Token;
    ...
    public function login()
    {
        ...
        if ($return['user_id'] > 0) {
            ...
            if (!$exist_user) {
                return error('登录失败，您的账号异常，请联系客服人员','',2002);
            }
    ...
    public function verificationCode()
    {
        if ($this->request->isPost()) {
            ...
            if ($SU && $res['code'] == 0) {
                // 发送短信
                return success('发送成功','',2000);
            } else {
                return error('发送失败','',2002);
            }
        } else {
            return error('请求方式错误','',4001);
        }
    }
        ...
        if ($thirdpartydata->user_id > 0) {//已经绑定过user_id
            ...
            if ($exist_user['code'] == '2000') {//用户存在
                if ($exist_user['data']['tel']) {
                    ...
                    if ($return['tel'] == $data['tel']) {
                        ...
                        $return['Authorization'] = $token;
                        return success('登录成功','',2000);
                    } else {
                        return error('您之前已经绑定过手机号，请重新登录','',2002);
                    }
                } else {
                    ...
                    if ($bind_tel['code'] == '2000') {
                        ...
                        $res                     = Cache::store('redis')->set('hguser:' . $token, $data_user, Config::get('app.user_token_exp'));
                        $return['Authorization'] = $token;
                        return success('登录成功',$return,2000);
                    } elseif ($bind_tel['code'] == '2001') {
                        //该手机号已经被其他人使用，需要重新输入手机号
                        return error($bind_tel['msg'],$return,3001);
                    } else {//其他情况，需要重新刷新页面
                        return error('系统异常，绑定失败',$return,2002);
                    }
                }
            } else {
                //user_id字段存在，但是user表用户不存在
                return error('账户异常',$return,2002);
            }
            ...
            if ($exist_user['code'] == '2000') {
                ...
                if ($register_tel['code'] == '2000') {
                    ...
                    $token                   = $this->getToken($return['user_id']);
                    $data_user['aud']        = 'user';
                    $data_user['uid']        = $return['user_id'];
                    $res                     = Cache::store('redis')->set('zuoyouweilai:' . $token, $data_user, Config::get('app.user_token_exp'));
                    $return['Authorization'] = $token;
